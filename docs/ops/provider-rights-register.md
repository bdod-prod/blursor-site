...
Only `production_authorized` surfaces may be scheduled or included in paid reports. `verification_only` data may be manually inspected for a client-owned property. `research_only` data must remain isolated from customer metrics and public methodology. `disabled` means no execution.

| Surface ID | Access method | State | Evidence and current conclusion | Next review or unblocker |
|---|---|---:|---|---|
| `yandex_gen_search_api_ru` | Official paid API | `production_authorized` after account setup | Official generative Search API and published per-request tariff. Must be labelled as the API surface, not consumer Alice. | Confirm account agreement, billing owner, quota, and production key before activation. Review monthly. |
...
| `gigachat_api` | Official commercial API | `production_authorized` after business setup | Official pay-as-you-go tariff exists. No evidence that API output equals every consumer GigaChat mode. | Confirm commercial terms, model retention settings, and exact model before activation. Review monthly. |
